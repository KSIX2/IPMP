...
void isMajority(int arr[], int size, int key){
    if(findFrequency(arr, size, key) > size/2)
        cout << "Yes the given key is a majority element!";
    else 
        cout << "Given key not a majority element!";
}

int main(){
    int arr[15] = {2,3,4,5,5,5,5,5,5,5,5,7,8,9,10};                               //sorted array
    int size = sizeof(arr)/sizeof(arr[0]);
    int key = 5;
    isMajority(arr, size, key);
    return 0;
}
